...
@Suite struct SystemPasteboardTests {
    ...
    private func makePasteboard() -> (SystemPasteboard, NSPasteboard) {
        let raw = NSPasteboard(name: NSPasteboard.Name("ss.test.\(UUID().uuidString)"))
        raw.clearContents()
    ...
    @Test func writePlainReplacesContentsInPlaceAndBumpsChangeCount() {
        let (pb, raw) = makePasteboard()
        raw.setData(Data("<p>rich</p>".utf8), forType: .html)
        let before = pb.changeCount

        let after = pb.writePlain("now plain only")
