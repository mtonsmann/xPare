...
| **In-memory only + wipe** | Content lives in memory only for the transform; pipeline intermediates, fused scratch buffers, and the buffer that crosses the boundary are wiped after use | The core holds each pipeline intermediate in a `Zeroizing` buffer (wiped on drop); fused pipeline scratch buffers are `Zeroizing` and wiped before reuse; `ss_buffer_free` zeroizes the returned buffer before freeing it (`zeroize` crate); `cargo xtask check-pipeline-zeroization` blocks the current fused-scratch regression class |
...
- **Zeroization is best-effort.** The core now holds each pipeline intermediate and
  fused scratch buffer that carries clipboard-derived bytes in `Zeroizing` storage
  and the FFI wipes the output buffer on free, so clipboard-derived content is
  scrubbed from the heap after use (at a measured throughput cost on very large
