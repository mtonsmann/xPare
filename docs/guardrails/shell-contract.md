...
1. **Clipboard read (incl. rich → plain).** Read the clipboard and **extract the best
   ...
   If the platform supports image OCR, keep it shell-owned: read bounded image
   bytes, run only local OS OCR, and write recognized plain text back only for a
   user-driven one-shot command or a separately enabled continuous-mode setting.
   Do not add OCR as a persistent core operation.
...
- For image OCR: confirmation that it is local, size-bounded, off by default for
  continuous mode unless the user separately enables it, and does not change the
  core ABI.
