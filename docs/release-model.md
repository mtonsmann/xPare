...
- `SHA256SUMS` — an aggregate checksum file written by `dist` into
  `dist/release/` and included in the one-shot draft release creation alongside
  the zip.
...
- An **SPDX-JSON SBOM** (`xPare-vX.Y.Z-sbom.spdx.json`, generated by syft over the
  workspace) included when the draft release is first created.
...
1. **validate** (always): checkout → require a successful Release Fuzz run on the
   ...
   workflow artifact.
2. **sign-official** (gated three ways): the repo variable
...
3. **attest-official**: uses the signed checksum subject list from `sign-official`
   ...
   `id-token: write`, `attestations: write`, and only `contents: read`; it does
   not download draft release assets or request artifact metadata write scope.
4. **sbom-official**: generates the SPDX-JSON SBOM as a workflow artifact with
   `contents: read` and `upload-release-assets: false`.
...
- `github-release` unpacks the final zip and runs `stapler validate` on the
  contained app **before** release creation, and refuses to run without the staged
  SBOM — an unstapled app or incomplete asset set never becomes a release asset.
...
syft dir:. -o spdx-json=dist/release/xPare-v1.0.0-sbom.spdx.json
