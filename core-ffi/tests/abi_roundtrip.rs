...
//! protocol, the lossy-UTF-8 input handling, and the `XpStatus` error model — and
...
use xpare_ffi::{
    xp_abi_version, xp_buffer_free, xp_capabilities_json, xp_transform, XpStatus,
    XP_MAX_INPUT_BYTES,
};
...
fn capabilities_str() -> &'static str {
    // SAFETY: `xp_capabilities_json` is documented to return a non-null pointer to
    ...
    let ptr = xp_capabilities_json();
    assert!(
        !ptr.is_null(),
        "xp_capabilities_json must never return null"
    );
...
/// Outcome of a successful `xp_transform`: the output bytes copied out of the
/// FFI-owned buffer, which has already been freed via `xp_buffer_free`.
struct TransformOk {
    status: XpStatus,
    bytes: Vec<u8>,
}

/// Drive `xp_transform` over the given input/config, copy the output buffer out,
/// then free it through `xp_buffer_free`. The returned `bytes` therefore outlive
...
fn transform(input: &[u8], cfg: &CStr) -> TransformOk {
    ...
    // local, non-null, and valid for writes. All preconditions of `xp_transform`
    // hold, so the call is sound.
    let status =
        unsafe { xp_transform(input_ptr, input.len(), cfg.as_ptr(), &mut out, &mut out_len) };

    let bytes = if status == XpStatus::Ok {
...
/// Free a buffer produced by `xp_transform` via the matching `xp_buffer_free`.
fn free(ptr: *mut u8, len: usize) {
    // SAFETY: `ptr`/`len` are exactly the pair `xp_transform` wrote into the
    // out-params for this call (or null/0, which `xp_buffer_free` treats as a
    // no-op). We free at most once per allocation and never use it afterwards.
    unsafe { xp_buffer_free(ptr, len) };
}
...
fn abi_version_is_three() {
    assert_eq!(xp_abi_version(), 3);
    // Cross-check against the public constant so a bump can never silently pass.
    assert_eq!(xp_abi_version(), xpare_ffi::XP_ABI_VERSION);
}
...
fn oversized_input_is_rejected_before_read_or_alloc() {
    ...
    // a tiny valid buffer with an over-limit `input_len`. `xp_transform` must return
    ...
    let mut out_len: usize = 7; // sentinel — must be cleared to 0 on error
                                // SAFETY: `input_len` > XP_MAX_INPUT_BYTES, so the size check returns before
    ...
    assert_eq!(status, XpStatus::ErrInputTooLarge);
    assert!(out.is_null(), "on error *out must be null");
...
fn capabilities_json_matches_core_and_describes_schema() {
    ...
    assert_eq!(
        ...
        "xp_capabilities_json must relay xpare_core::capabilities() verbatim"
    );
...
fn transform_happy_path_strip_html_then_collapse_whitespace() {
    ...
    assert_eq!(result.status, XpStatus::Ok);
...
fn transform_empty_null_input_is_ok_and_freeable() {
    ...
    let status = unsafe { xp_transform(std::ptr::null(), 0, cfg.as_ptr(), &mut out, &mut out_len) };

    assert_eq!(status, XpStatus::Ok);
    ...
    // safe to hand to xp_buffer_free exactly once.
    let copied = copy_out(out, out_len);
...
/// Invoke `xp_transform` with raw arguments and assert it returns `expected`,
/// leaving `*out`/`*out_len` cleared. Used for the null/invalid-arg cases where we
/// must control each pointer individually.
///
/// # Safety
/// `input`/`config_json` must satisfy `xp_transform`'s preconditions *for the case
...
unsafe fn assert_transform_err(
    ...
    let status = unsafe { xp_transform(input, input_len, config_json, &mut out, &mut out_len) };
...
fn transform_null_config_is_null_arg() {
    ...
    unsafe {
        assert_transform_err(b"x".as_ptr(), 1, std::ptr::null(), XpStatus::ErrNullArg);
    }
}
...
fn transform_null_out_pointer_is_null_arg() {
    ...
    assert_eq!(status, XpStatus::ErrNullArg);
}
...
fn transform_null_input_with_nonzero_len_is_null_arg() {
    ...
    unsafe {
        assert_transform_err(std::ptr::null(), 4, cfg.as_ptr(), XpStatus::ErrNullArg);
    }
}
...
#[test]
fn transform_unsupported_version_is_its_own_status() {
    // A config schema-version mismatch must surface as the dedicated ABI v3 status
    // — value 5, distinct from malformed JSON's ErrInvalidConfig (2) — so a shell
    // can tell upgrade/downgrade skew apart from a broken config.
    let cfg = config(r#"{"version":999,"operations":[]}"#);
    let result = transform(b"hello", &cfg);
    assert_eq!(result.status, XpStatus::ErrUnsupportedConfigVersion);
    assert!(result.bytes.is_empty());

    // Pin the C-visible discriminants this distinction rides on.
    assert_eq!(XpStatus::ErrUnsupportedConfigVersion as u32, 5);
    assert_eq!(XpStatus::ErrInvalidConfig as u32, 2);
    assert_ne!(
        result.status,
        XpStatus::ErrInvalidConfig,
        "version mismatch must be distinguishable from malformed JSON"
    );
}
...
// ---------------------------------------------------------------------------
// 6b. xp_buffer_free contract: a null pointer is a documented no-op (so a caller
